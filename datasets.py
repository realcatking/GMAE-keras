...
def extract_Resnet50_features(x):
    ...
    # im_h = x.shape[1]
    im_h = 224
    model = ResNet50(include_top=False, weights='imagenet', pooling='avg', input_shape=(im_h, im_h, 3))
    # if flatten:
    #     add_layer = Flatten()
    # else:
    #     add_layer = GlobalMaxPool2D()
...
def make_reuters_data(data_dir, num_samples=None):
    np.random.seed(1234)
    ...
    if num_samples:
        ...
        np.savez(join(data_dir, 'reutersidf.npz'), data=x, label=y)
...
def load_usps_mat(data_path='./data/usps'):
    data=scio.loadmat(data_path+'/usps_all.mat')
    X = data['data'].transpose((0,2,1)).reshape(256,-1).T
    X = X.astype('float64')
    X = (X-X.min())/X.max()
    Y=np.zeros(11000)
    for i in range(10):
...
def load_reuters10k(data_path='./data/reuters'):
    import os
    if not os.path.exists(os.path.join(data_path, 'reutersidf10k.npy')):
        print('making reuters idf features')
        make_reuters_data(data_path, 10000)
...
def load_reuters(data_path='./data/reuters'):
    import os
    if not os.path.exists(os.path.join(data_path, 'reutersidf.npz')):
        print('making reuters idf features')
        make_reuters_data(data_path)
        print(('reutersidf saved to ' + data_path))
    data = np.load(os.path.join(data_path, 'reutersidf.npz'), allow_pickle=True)
    # has been shuffled
    x = data['data']
    y = data['label']
    x = x.reshape((x.shape[0], -1)).astype('float64')
    y = y.reshape((y.size,))
    print(('REUTERSIDF samples', x.shape))
    return x, y
...
def load_stl(data_path='./data/stl'):
    ...
    features = extract_Resnet50_features(x)
...
def load_har(data_path='./data/har'):
    data=scio.loadmat(data_path+'/HAR.mat')
    X=data['X']
    X=X.astype('float32')
    # X =(X+1)/2.
    Y=data['Y']-1
...
def load_data(dataset_name):
    if dataset_name == 'mnist':
        ...
        return load_har()
    elif dataset_name == 'usps_mat':
        return load_usps_mat()
    else:
